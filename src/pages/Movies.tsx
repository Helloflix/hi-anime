...
const Movies = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      setLoading(true);
      try {
        // Fetch films category (2) from PeerTube
        const data = await getVideos(0, 100, sortBy, [PT_CATEGORIES.FILMS]);
        setVideos(data.data || []);
      } catch (error) {
        console.error("Failed to fetch movies:", error);
        // If category filter returns nothing, fetch all and let user browse
        try {
  ...
  const filteredMovies = videos.filter(v =>
    ...
    <div className="container px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Browse movies from your PeerTube library
        </p>
      </div>
      ...
      {!loading && (
        <>
          <div className="mb-6">
            <p className="text-muted-foreground">
              Showing {filteredMovies.length} of {videos.length} results
            </p>
          </div>
